...
const STRUCTURED_TEST_MAX_OUTPUT_TOKENS = 8000;
...
function testResponseFormat(targetQuestionCount = null) {
  const minimum = targetQuestionCount || 8;
  const maximum = targetQuestionCount || 10;
  return {
    type: "json_schema",
    json_schema: {
      name: "generated_school_test",
      strict: true,
      schema: {
        type: "object",
        additionalProperties: false,
        properties: {
          title: { type: "string", description: "Краткий заголовок теста без Markdown-символов заголовка." },
          questions: {
            type: "array",
            minItems: minimum,
            maxItems: maximum,
            items: {
              ...
              additionalProperties: false,
              properties: {
                prompt: { type: "string", description: "Полное однозначное условие вопроса." },
                options: {
                  type: "object",
                  additionalProperties: false,
                  description: "Ровно четыре попарно неравносильных варианта. Запрещены одинаковые значения в разных записях, тождественные выражения и одинаковые по смыслу формулировки.",
                  properties: {
                    A: { type: "string" },
                    B: { type: "string" },
                    C: { type: "string" },
                ...
                explanation: { type: "string", description: "Завершённое объяснение, доказывающее именно выбранный вариант." },
              },
...
function structuredTestError(message) {
  const error = new Error(message);
  error.code = "AI_STRUCTURED_OUTPUT_INVALID";
  return error;
}

function parseStructuredTest(text, { targetQuestionCount = null } = {}) {
  let parsed;
  try {
    parsed = JSON.parse(String(text || ""));
  } catch {
    throw structuredTestError("AI returned invalid structured test JSON");
  }
  if (!parsed || typeof parsed !== "object" || !Array.isArray(parsed.questions)) {
    throw structuredTestError("AI returned an invalid structured test contract");
  }
  const minimum = targetQuestionCount || 8;
  const maximum = targetQuestionCount || 10;
  if (parsed.questions.length < minimum || parsed.questions.length > maximum) {
    throw structuredTestError("AI returned an invalid number of test questions");
  }
  for (const question of parsed.questions) {
    if (!question || typeof question.prompt !== "string" || typeof question.explanation !== "string") {
      throw structuredTestError("AI returned an incomplete test question");
    }
    if (!question.options || !TEST_OPTION_LABELS.every((label) => typeof question.options[label] === "string" && question.options[label].trim())) {
      throw structuredTestError("AI returned incomplete test options");
    }
    if (!TEST_OPTION_LABELS.includes(question.correctAnswer)) {
      throw structuredTestError("AI returned an invalid answer key label");
    }
    const uniqueOptions = new Set(TEST_OPTION_LABELS.map((label) => question.options[label].replace(/\s+/g, " ").trim()));
    if (uniqueOptions.size !== TEST_OPTION_LABELS.length) {
      throw structuredTestError("AI returned duplicate test options");
    }
  }
  return parsed;
}

function renderStructuredTest(test) {
  const title = String(test.title || "Тест").replace(/^#+\s*/, "").trim() || "Тест";
  const questions = test.questions.map((question, index) => [
    `${index + 1}. ${question.prompt.trim()}`,
    ...TEST_OPTION_LABELS.map((label) => `${label}. ${question.options[label].trim()}`),
  ].join("\n")).join("\n\n");
  const answers = test.questions.map((question, index) => (
    `${index + 1}. ${question.correctAnswer} — ${question.explanation.trim()}`
  )).join("\n");
  return `# ${title}\n\n${questions}\n\n## ${ANSWERS_MARKER}\n${answers}`;
}
...
const TEST_OPTION_EQUIVALENCE_INSTRUCTION = `В каждом вопросе варианты A, B, C и D должны быть попарно неравносильными, а не просто по-разному записанными. Перед выдачей сравни каждую пару вариантов после вычисления, упрощения и приведения к единой форме. Считай повтором и обязательно заменяй: равные десятичные и обычные дроби (например, 2,5; 2.5; 5/2; 10/4), равные проценты и дроби, тождественно равные алгебраические выражения на указанном ОДЗ, одинаковые множества или промежутки в разной записи, а также синонимичные формулировки с одним смыслом. Неверные варианты тоже обязаны отличаться друг от друга по значению или смыслу. Ровно один вариант должен быть правильным.`;
...
const INTERNAL_REVISION_PATTERN = /\[(?:корректировк|финальн[а-яё]*\s+верси)[^\]]*\]|\b(?:ой,?\s+ошибка|ошибка\s+в\s+(?:вопросе|условии|вычислениях|решении)|давайте\s+(?:изменим|заменим|верн[её]мся)|верн[её]мся\s+к\s+исходному|новый\s+вопрос)\b/iu;
...
function richMarkdownIssues({ text, type, student, topic }) {
  ...
  if (PROCESS_NOTE_PATTERN.test(value) || INTERNAL_REVISION_PATTERN.test(value)) issues.push("есть служебный комментарий о проверке или исправлениях");
...
class ContentGenerator {
  ...
  buildMessages({ type, student, card, topic, adjustment, previousResult, customInstruction, targetQuestionCount }) {
    ...
    const instructions = [
      ...
      type === "test" ? TEST_OPTION_EQUIVALENCE_INSTRUCTION : "",
      type === "test" ? "Верни тест строго в запрошенной структурированной схеме. Не помещай метки A–D внутрь текста вариантов и не добавляй служебные рассуждения, черновики или исправления." : "",
  ...
  async complete(messages, { model = this.model, reasoningEffort = null, timeoutMs = null, responseFormat = null } = {}) {
    ...
    if (this.provider === "openrouter") {
      ...
      const response = await this.client.chat.completions.create({
        ...
        max_tokens: responseFormat?.json_schema?.name === "generated_school_test"
          ? STRUCTURED_TEST_MAX_OUTPUT_TOKENS
      ...
      const response = await this.client.responses.create({
        ...
        ...(responseFormat && { text: { format: { type: "json_schema", ...responseFormat.json_schema } } }),
      }, timeoutMs ? { timeout: timeoutMs } : undefined);
  ...
  */
  async generate({ type, student, card, topic, adjustment = null, previousResult = null, customInstruction = null, onProgress = null }) {
    ...
    const responseFormat = type === "test" ? testResponseFormat(targetQuestionCount) : null;
    let rawResult = await this.completeWithProviderRetry(messages, { reasoningEffort: "high", responseFormat });
    let result = type === "test"
      ? renderStructuredTest(parseStructuredTest(rawResult, { targetQuestionCount }))
      : rawResult;
    ...
    if (issues.length) {
      const correction = type === "test"
        ? "Перепиши предыдущий тест полностью по запрошенной структурированной схеме. Для каждого вопроса заново проверь условие, все четыре разных варианта, correctAnswer и explanation. Не описывай процесс проверки и не меняй условие внутри объяснения. LaTeX-команды в строках должны находиться внутри парных $...$ или $$...$$."
      ...
      result = await this.complete([
        ...messages,
        { role: "assistant", content: type === "test" ? rawResult : initialResult },
        { role: "user", content: `${correction}\nАвтоматическая проверка обнаружила: ${issues.join("; ")}` },
      ], { reasoningEffort: "high", responseFormat });
      if (type === "test") {
        rawResult = result;
        result = renderStructuredTest(parseStructuredTest(result, { targetQuestionCount }));
      }
